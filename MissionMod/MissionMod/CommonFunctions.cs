...
namespace MissionMod
{
    class CommonFunctions
    {
        ...
        internal static void DebugAPI2(string ProcessName, string FileData)
        {
            if (MyEmpyrionMod.debug)
            {
                LogFile(ProcessName + ".txt", FileData);
            }
        }
        ...
        public static ItemStack[] ReadItemStacks(string File)
        {
            ...
            for (int i = 0; i < itemStackSize; ++i)
            {
                ...
                itStack[i] = new ItemStack
                {
                    ...
                    id = Convert.ToInt32(bagLinesSplit[1]),
                    count = Convert.ToInt32(bagLinesSplit[2]),
